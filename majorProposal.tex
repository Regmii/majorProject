...
The 'best1bin' strategy is a good starting point binomial distribution.
...
The trial vector \(u_i\) competes with the original solution \(x_i\), and the better one (based on fitness function) moves to the next generation.
\begin{equation}
x_i' =
\left\{
\begin{array}{l}
    u_i, {if} \quad f(u_i) \leq f(x_i) \\[5pt]
    x_i, {otherwise.}
\end{array}
\right.
\end{equation}
where \(f(x)\) is the objective function (lower is better in minimization problems).
If we are minimizing a function, and:
\begin{equation}
f(x) = x_1^2 + x_2^2
\end{equation}
\begin{itemize}
    \item \( f(2,3) = 2^2 + 3^2 = 13 \)
    \item \( f(2,2.5) = 2^2 + 2.5^2 = 10.25 \)
\end{itemize}
Since \(10.25 < 13\), we keep \([2, 2.5]\) as the new solution.\\
